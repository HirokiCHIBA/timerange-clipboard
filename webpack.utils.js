...
class PageModules {
  ...
  constructor(rootDir) {
    ...
    this.entries = Object.fromEntries(
      this.keys.map((key) => [key, path.resolve(this.#vModuleDir, `${key}.tsx`)])
    )
  }
...
<html>
  ...
  getRendererModules() {
    return Object.fromEntries(
      this.keys.map((key) => [this.entries[key], `
...
`
  ...
  static async generate(packages, textOverrides) {
    const overrides = await Promise.all(textOverrides.map(async e => {
      const response = await fetch(e.textUrl);
      if (!response.ok) {
        throw new Error(`failed to fetch ${e.textUrl} for ${e.namePrefix}: ${response.statusText}`);
      }
      e["licenseText"] = await response.text();
      return e;
    }))
    return this.#header + packages.map(p => {
